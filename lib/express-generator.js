...
const { pathExists } = require('./utilities');
...
async function generateApp(destination, cliAnswers) {
  ...
  try {
    // Check if project folder already exists otherwise create folder on destionation
    ...
    // Copy basic project folder/files structure
    await copyBaseFiles(destination, cliAnswers);

    // Generating package.json ( npm init -y )
    ...
    // Update name, description and scripts on the package.json
    ...
    try {
      // copy route file for Route option
    ...
    // copy controller for mongoDb option ( rxjs or not ).
    if (cliAnswers.withMongo) {
      await copyMongoFilesCliOptions(destination, cliAnswers);
    }

    // copy Auth route file and edit the route/index.ts to add the auth route.
    ...
    // Get the dependencies string and execute 'npm i -s' and 'npm i -D'
...
function getDependencies(cliAnswers) {
  let dependencies =
    'express helmet morgan winston dotenv cors autobind-decorator http-errors';
  let devDependencies =
