...
function copyAuthFile(destination, spinner, cliAnswers) {
  ...
  const updateIndexRoute = new Promise((resolve, reject) => {
    ...
    fs.readFile(filePath, (err, data) => {
      ...
      const position = fileString.indexOf('userRoute);');
      let start = fileString.slice(0, position + 11);
      let end = fileString.slice(position + 11);
      const newFileString = `import authRoute from './auth.route';\n${start}\n\nrouter.use('/auth', authRoute);${end}`;
